...
# funksjon som bruker eulers metode
def euler(arr_1, arr_2, i, dx):
...
for i in range(len(t)):
    v_x.append(euler(v_x, a_x, i, dt)) # legger til fart i fartlista med Eulers metode
    v_y.append(euler(v_y, a_y, i, dt)) # legger til fart i fartlista med Eulers metode
    s_x.append(euler(s_x, v_x, i, dt)) # legger til strekning i strekninglista med Eulers metode
    s_y.append(euler(s_y, v_y, i, dt)) # legger til strekning i strekninglista med Eulers metode
    a.append(lengde_vektor(a_x, a_y, i)) # regner ut den totale akselerasjonen og legger den til i lista
    v.append(lengde_vektor(v_x, v_y, i)) # regner ut den totale farten og legger den til i lista
...
fig, axs = plt.subplots(1, 2, constrained_layout=True, figsize=(12, 5)) # lager subplotene og definerer størrelsen på de
